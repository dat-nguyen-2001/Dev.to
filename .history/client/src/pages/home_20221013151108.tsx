...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    getLikedArticles().then(data => setLikedArticles(data)).then(() => setIsLiked(likedArticles.includes(article.id)))
        ...
        <div className='col-span-5 px-2'>
          ...
          <div className='flex flex-col space-y-2'>
            {articles.map(article => {
              return (
                <div key={article.id}>
                  <ArticleBlock article={article} isLiked = {likedArticles.includes(article.id)}/>
                </div>
              )
            })}
          </div>
