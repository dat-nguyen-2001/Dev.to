...
const ArticlePage = () => {
    ...
    console.log(likedArticles.includes(Number(article.id)))
    const [isLiked, setIsLiked] = useState<Boolean>(article !== undefined ? likedArticles.includes(Number(article.id)) : false)
    const handleLikeArticle = async() => {
        if(!username) {
            return navigate('/enter')
        }
        setIsLiked(prevState => !prevState);
        const articleId = article !== undefined ? Number(article.id) : 0
